...
# 27.02.12 Bondar A.G. Corrected a mistake in the assign_macro_variable() function 
#
#
# FUNCTION                                        DESCRIPTION                                                RETURN
# assign_macro_data(arg_str)                      Assign data to corresponding macros                        NONE
# append_arg_str(arg_str, val1,val2)              Append value to a string                                   arg_str
# get_token(p_string, p_fs, p_idx) )              Retrieve a token from the document                         p_string[l_start, l_end] or NULL
# assign_macro_variable(var_name,var_value)       Assign value to macro                                      NONE
# process_html_string(html_text_in)               Process html string                                        html_return
#########################################################################################################
...
#
# Assign data to corresponding macros
#
# RETURN NONE
##########################################################################################
FUNCTION assign_macro_data(arg_str)
...
#
# Retrieve a token from the document
#
# RETURN p_string[l_start, l_end] or NULL
##########################################################################################
FUNCTION get_token(p_string, p_fs, p_idx) 
  DEFINE 
    p_string CHAR(5000),
  ...
  FOR i = 1 TO l_len
    IF p_string[i,i+1] = p_fs THEN
      ...
      LET l_end = i - 1
      IF l_idx = p_idx THEN
      	IF l_start - l_end > 0 THEN
      		#we have situation when no value between ,;,; delimiters
			#then we should return empty value
      		RETURN " "
      	ELSE
     ...
     END IF
    IF local_debug = 1  THEN
      DISPLAY "token " ,trim(i), " - length", l_len
      DISPLAY " l_start=", trim(l_start), " l_end=", trim(l_end)
      DISPLAY "p_string[l_start, l_end]=", p_string[l_start, l_end]  
      DISPLAY "LENGTH(p_string)=", trim(LENGTH(p_string))
      DISPLAY "l_idx =", trim(l_idx) , " p_idx=", trim(p_idx)
      DISPLAY "p_string[l_start, l_end]   = ", p_string[l_start, l_end]
      DISPLAY "p_string[i,i+1]=", p_string[i,i+1] , " p_fs= ", p_fs
      DISPLAY "----------------------------------------------"  
      #sleep 1
    END IF
...
## END OF Replace CR/LF with <br> ##
  ...
  CASE var_name
    ...
    WHEN "hm_s22"
      LET hm_s[22] = var_value
    WHEN "hm_s23"
      LET hm_s[23] = var_value
    WHEN "hm_s24"
      LET hm_s[24] = var_value
    WHEN "hm_s25"
      LET hm_s[25] = var_value
    WHEN "hm_s26"
      LET hm_s[26] = var_value
    WHEN "hm_s27"
      LET hm_s[27] = var_value
    WHEN "hm_s28"
      LET hm_s[28] = var_value
    WHEN "hm_s29"
      LET hm_s[29] = var_value
    WHEN "hm_s30"
      LET hm_s[30] = var_value
    WHEN "hm_s31"
      LET hm_s[31] = var_value
    WHEN "hm_s32"
      LET hm_s[32] = var_value
    WHEN "hm_s33"
      LET hm_s[33] = var_value
    WHEN "hm_s34"
      LET hm_s[34] = var_value
    WHEN "hm_s35"
      LET hm_s[35] = var_value
    WHEN "hm_s36"
      LET hm_s[36] = var_value
    WHEN "hm_s37"
      LET hm_s[37] = var_value
    WHEN "hm_s38"
      LET hm_s[38] = var_value
    WHEN "hm_s39"
      LET hm_s[39] = var_value
    WHEN "hm_s40"
      LET hm_s[40] = var_value
    WHEN "hm_n01"
  ...
  OTHERWISE
    LET err_msg = "assign_macro_variable()\nCase condition variable var_name = ", var_name 
    CALL fgl_winmessage("Internal Demo 4GL Source Code Error", err_msg, "error")
  ...
  FOR ci = 1 TO file_length
    ...
    IF token[1,4] = "@@qx" AND token[8,9] = "@@" THEN
      ...
      CASE token_type
        ...
        OTHERWISE
          LET err_message = "Error in function process_html_string()\n'CASE Condition token_type =", token_type
          CALL fgl_winmessage("Internal Demo 4GL Source Code error",err_message, "error")
