...
MAIN
   ...
   CREATE VIEW lyc_7812_view_1_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz AS SELECT * FROM lyc_7812_table 
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "2. SQLCA.SQLCODE = ",SQLCA.SQLCODE EXIT PROGRAM END IF

	SELECT tabname INTO v_tabname FROM systables WHERE tabname = "lyc_7812_view_1_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
   ...
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "4. SQLCA.SQLCODE = ",SQLCA.SQLCODE ELSE DISPLAY "4. ",v_tabname END IF
	...
	LET v_prep = "CREATE VIEW lyc_7812_view_2_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz AS SELECT * FROM lyc_7812_table"
   PREPARE p_create FROM v_prep
   EXECUTE p_create
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "5. SQLCA.SQLCODE = ",SQLCA.SQLCODE EXIT PROGRAM END IF

	SELECT tabname INTO v_tabname FROM systables WHERE tabname = "lyc_7812_view_2_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
   ...
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "7. SQLCA.SQLCODE = ",SQLCA.SQLCODE ELSE DISPLAY "7. ",v_tabname END IF
	...
	SQL
   CREATE VIEW lyc_7812_view_3_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz AS SELECT * FROM lyc_7812_table
	END SQL

	SELECT tabname INTO v_tabname FROM systables WHERE tabname = "lyc_7812_view_3_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
