...
MAIN
   ...
   DROP TABLE lyc_1109
   CREATE TABLE lyc_1109
   (col_byte BYTE
   )
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY SQLCA.SQLCODE END IF

   DECLARE cur CURSOR FOR
      SELECT column_name,data_type,character_maximum_length FROM information_schema.columns WHERE information_schema.columns.table_name='lyc_1109'
