...
MAIN
   ...
   DROP TABLE lyc_7865_select_when_error_3
   CREATE TABLE lyc_7865_select_when_error_3
   (row_number INTEGER,
    col_char CHAR(80)
   )

   INSERT INTO lyc_7865_select_when_error_3 VALUES(1,"1st row")
   INSERT INTO lyc_7865_select_when_error_3 VALUES(2,"2nd row")
   INSERT INTO lyc_7865_select_when_error_3 VALUES(3,"3rd row")
   INSERT INTO lyc_7865_select_when_error_3 VALUES(4,"4th row")
   INSERT INTO lyc_7865_select_when_error_3 VALUES(5,"5th row")
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "1. SQLCA.SQLCODE = ",SQLCA.SQLCODE EXIT PROGRAM END IF

	SQL SELECT row_number,CASE WHEN row_number = 1 THEN "1st row" ELSE "NULL" END INTO $v_row_number,$v_char FROM lyc_7865_select_when_error_3 WHERE row_number = 1 END SQL
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "2. SQLCA.SQLCODE = ",SQLCA.SQLCODE ELSE DISPLAY "2. ", v_row_number,"  ",v_char END IF

	SQL SELECT row_number,CASE WHEN row_number = 1 THEN "1st row" ELSE "NULL" END INTO $v_row_number,$v_char FROM lyc_7865_select_when_error_3 WHERE row_number = 5 END SQL
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "3. SQLCA.SQLCODE = ",SQLCA.SQLCODE ELSE DISPLAY "3. ", v_row_number,"  ",v_char END IF
	...
	SQL SELECT row_number,
				  CASE WHEN row_number = 1 THEN "1st row"
						 WHEN row_number = 2 THEN "2nd row" 
						 WHEN row_number = 3 THEN "3rd row"
						 WHEN row_number = 4 THEN "4th row"
						 WHEN row_number = 5 THEN "5th row"
						 ELSE "NULL"
				  END
				  INTO $v_row_number,$v_char	
				  FROM lyc_7865_select_when_error_3 WHERE row_number = 1
	END SQL				  
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "4. SQLCA.SQLCODE = ",SQLCA.SQLCODE ELSE DISPLAY "4. ", v_row_number,"  ",v_char END IF
	...
	SQL SELECT row_number, 
				  ...
				  END
				  INTO $v_row_number,$v_char
		 FROM lyc_7865_select_when_error_3 WHERE row_number = 5
	END SQL		 
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "5. SQLCA.SQLCODE = ",SQLCA.SQLCODE ELSE DISPLAY "5. ", v_row_number,"  ",v_char END IF
	...
	SQL SELECT row_number,
				  ...
				  END
				  INTO $v_row_number,$v_char
		 FROM lyc_7865_select_when_error_3 WHERE row_number = 0
	END SQL
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "6. SQLCA.SQLCODE = ",SQLCA.SQLCODE ELSE DISPLAY "6. ", v_row_number,"  ",v_char END IF
	...
	SQL SELECT row_number,
				  ...
				  END
				  INTO $v_row_number,$v_char
		 FROM lyc_7865_select_when_error_3
	END SQL
		...
		SQL SELECT row_number, 
					  CASE WHEN row_number = 1 THEN "1st row"
							 ...
							 WHEN row_number = 5 THEN "5th row"
						 	 ELSE "NULL"
				  	  END
		 	FROM lyc_7865_select_when_error_3 ORDER BY row_number
		END SQL
   ...
   DECLARE cursor_4 CURSOR FOR
	SQL SELECT row_number, 
	    		  ...
	    		  END
	    INTO $v_row_number,$v_char
		 FROM lyc_7865_select_when_error_3 WHERE row_number BETWEEN 2 AND 4 ORDER BY row_number
	END SQL
   FOREACH cursor_4
