...
MAIN
	...
	OPEN WINDOW wMain WITH FORM "form/sta_input_array_no_append"
	CALL ui.Interface.setText("INPUT ARRAY No Append")
  ...
  INPUT ARRAY cntRecArr WITHOUT DEFAULTS FROM sArr.*
     ATTRIBUTES(UNBUFFERED, APPEND ROW = false, auto append = false)
     #ATTRIBUTE(UNBUFFERED, APPEND ROW = 1, INSERT ROW = 0, DELETE ROW = 1, AUTO APPEND = 0)
     BEFORE ROW
     ...
     BEFORE INSERT  
		...
		DISPLAY "cntRecArr.getLength()=", cntRecArr.getLength()  		        
     AFTER INSERT  
			LET lArrCurr = arr_curr()           
...
END FUNCTION
##################################################################################
# END FUNCTION displayInformation(fNewLine)
##################################################################################   
