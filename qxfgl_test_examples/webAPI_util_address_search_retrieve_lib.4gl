...
MAIN
	...
	DEFINE l_rec_addr OF UTIL_ADDRESS_RETRIEVE_OUTPUT 								#Final field based Address	 #internal lycia_globals.4gl defined record structure
	...
	DEFINE l_rec_addres OF UTIL_ADDRESS_RETRIEVE_OUTPUT #internal lycia_globals.4gl defined record structure
	...
	CALL main_address() RETURNING l_status, l_rec_addr.*
	...
	IF l_status THEN
		LET l_msg = 
			...
			l_rec_addr.Line4, ",", 
			l_rec_addr.Line5  
		CALL fgl_winmessage("Address was returned",l_msg,"info")
	END IF
	
{
					...
					CASE l_arr_rec_addr_filtered[l_idx].type 
						WHEN "Postcode"							
							...
							l_arr_rec_addr_filtered[1].type = "Address" THEN 
								CALL util_address_retrieve(l_arr_rec_addr_filtered[1].id,l_rec_param.license_key) RETURNING l_rec_addr.*
								CALL display_address(l_rec_addr.*)
							END IF	

						WHEN "Address"
							CALL util_address_retrieve(l_arr_rec_addr_filtered[l_idx].id,l_rec_param.license_key) RETURNING l_rec_addr.*
							CALL display_address(l_rec_addr.*)
	...
	#DIALOG ACTIONS ---------------------------------------------------------------------------
		...
		ON ACTION "RESULT"
			CALL util_address_retrieve(l_retrieve_address_id,l_rec_param.license_key)
