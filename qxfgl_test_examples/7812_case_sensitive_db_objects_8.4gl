...
MAIN
   ...
   (row_number INTEGER CHECK (row_number BETWEEN 0 AND 99999) CONSTRAINT lyc_7812_table_column_row_number_check_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz,
    col_char CHAR(80)
   )
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "1. SQLCA.SQLCODE = ",SQLCA.SQLCODE EXIT PROGRAM END IF

	SELECT constrname INTO v_constrname FROM sysconstraints WHERE constrname = "lyc_7812_table_column_row_number_check_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
