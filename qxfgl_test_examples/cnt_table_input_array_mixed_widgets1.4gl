...
FUNCTION hubert()
	...
	IF 1=1 THEN
		display "hello line 1"
		CALL ui.dialog.getcurrent().next_field("lname") 
		display "hello line 2"
		display "hello line 3"
	END IF
END FUNCTION
######################################################################
# MAIN
...
MAIN
	...
	OPEN WINDOW wContact WITH FORM "form/cnt_table_input_array_mixed_widgets1"
  CALL fgl_settitle("Table Container Input Array with mixed widgets Demo Example 1")
	CALL ui.Interface.setText("InpArr")
	DISPLAY "Input Array with mixed widgets - Demo 1" TO lbTitle
	
	CALL init_data()

	DISPLAY arrRec_contactMorphed.getlength() TO lbArrCnt
	
	INPUT ARRAY arrRec_contactMorphed WITHOUT DEFAULTS FROM scr_contact1.* ATTRIBUTE(UNBUFFERED)
		BEFORE INPUT
...
FUNCTION morph_contactRec(id,p_rec_guidemo_cust)
	...
	#LET arrRec_contactMorphed[id].gender = rec_contact.gender
