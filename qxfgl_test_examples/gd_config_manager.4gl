...
# www.bbf7.de
#
# Please visit this for licencing information before you re-use this code
################################################################################
...

{
 ...
 Aufrufbare Funktionen
	function start_kunden()
		- Startet die kundenverwaltung
		global return
			--
	function configInit(_file)
		-	initialisiert das modul config
			Muss einmal aufgrufen werden.
		- 	parameter
				_ptr		der Dateinamen mit abs. oder rel. Pfad
...
##########################################################################################
FUNCTION _isVariableDefined(_name)
  DEFINE _name      VARCHAR(100)
  DEFINE p1, p2, i  SMALLINT
	
  LET p1 = 0
  LET p2 = 0

  FOR i = 1 TO length(_name)
    IF _name[i,i+1] = "%(" THEN
      LET p1 = i
    END IF
...
FUNCTION configWrite(_ptr, _section, _name, _value)
  ...
  IF not arrpos THEN
    ...
    LET arrpos = _searchInsertPos(_ptr, _section)		-- position suchen
    #CALL _makeFreePosInArray(_ptr, arrpos)				-- position frei machen
  END IF
  ...
  FOR i = 1 TO config_sys[_ptr].last_position_in_array 
    ...
    IF tmp_rec.nname[1] = '#' or length(tmp_rec.nname) = 0 THEN
      LET tmp_rec.deli = ' '
    ELSE
      LET tmp_rec.deli = ' = '
    END IF
...
FUNCTION configRead(_ptr)
  ...
  WHILE fgl_channel_read("stream",temp_rec.*)
    ...
    # test ob es ein section parameter ist
