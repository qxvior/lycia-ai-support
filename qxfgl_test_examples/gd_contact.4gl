    ...
    BEFORE DISPLAY
      ...
      ON KEY(F5)    -- Edit Data
        ...
        END IF
			...
			ON ACTION ("actInputData")  --"actInputDataTb"
        CALL input_data()
        IF exit_menu = 1 THEN
          EXIT WHILE  
        END IF
     ...
     #   CALL image_selection()  --click the persons picture
      ...
      # commented by seza 19th Apr
      #Vlad is trying to fix grid_export()
      #ON KEY (F83)
      ...
      ON ACTION ("actPrint")    -- just to show you can work with key or action events
...
FUNCTION manage_list_box()
  ...
  IF list_value = "" THEN --IS NULL THEN
...
FUNCTION populate_form()
  ...
  DISPLAY get_str(215) TO fb_input
  DISPLAY get_str(216)TO fb_done
...
FUNCTION country_popup()
  ...
  CALL w.OpenWithForm("w_popup_country", get_form_path("gd_f_country_lookup_2_2"), 8, 20,  "border")
...
RETURN "gd_f_contact_english_2_2"
{
  CASE get_language()  --global
    WHEN 1
      RETURN "gd_f_contact_english_2_2"
