...
#####################################################################################
GLOBALS "webAPI_util_address_search_retrieve_globals.4gl"
GLOBALS "webAPI_loqate_license_globals.4gl"

DEFINE dt_arr_UTIL_ADDRESS_RETRIEVE_OUTPUT TYPE AS DYNAMIC ARRAY OF UTIL_ADDRESS_RETRIEVE_OUTPUT
...
FUNCTION main_address()
	DEFINE ret_status SMALLINT #return status
...
#	DEFINE l_rec_addr OF UTIL_ADDRESS_RETRIEVE_OUTPUT #internal lycia_globals.4gl defined record structure
	DEFINE l_rec_addr_tmp_arr OF dt_arr_UTIL_ADDRESS_RETRIEVE_OUTPUT 
	...
	DEFINE l_rec_addr OF UTIL_ADDRESS_RETRIEVE_OUTPUT
	DEFER INTERRUPT
	OPTIONS INPUT WRAP
	...
	OPEN WINDOW w_addressManager WITH FORM "form/webAPI_util_address_search_retrieve_lib"
					...
					CASE l_arr_rec_addr_filtered[l_idx].type 
						WHEN "Postcode"							
							...
							l_arr_rec_addr_filtered[1].type = "Address" THEN 
								CALL util_address_retrieve(l_arr_rec_addr_filtered[1].id,l_rec_param.license_key) RETURNING l_rec_addr_tmp_arr[1].* #RETURNING l_rec_addr.*
									LET l_rec_addr.* = l_rec_addr_tmp_arr[1].*  
								CALL display_address(l_rec_addr.*)
							END IF	

						WHEN "Address"
							CALL util_address_retrieve(l_arr_rec_addr_filtered[l_idx].id,l_rec_param.license_key)  RETURNING l_rec_addr_tmp_arr[1].* #RETURNING l_rec_addr.*
									LET l_rec_addr.* = l_rec_addr_tmp_arr[1].* 
							CALL display_address(l_rec_addr.*)
	...
	#DIALOG ACTIONS ---------------------------------------------------------------------------
		...
		ON ACTION "RESULT"
			LET l_ready = FALSE
			CALL util_address_retrieve(l_retrieve_address_id,l_rec_param.license_key)  RETURNING l_rec_addr_tmp_arr#[1].* #RETURNING l_rec_addr.*
			LET l_rec_addr.* = l_rec_addr_tmp_arr[1].* 
	...
	RETURN ret_status, l_rec_addr.*
END FUNCTION
#####################################################################################
# END FUNTION main_address
#####################################################################################
...
FUNCTION form_search_text(p_rec_search)
	...
	IF p_rec_search.text2 IS NOT NULL THEN
		...
		ELSE
			LET l_ret_tmp_str = trim(l_ret_tmp_str), "|", trim(p_rec_search.text2)
		END IF
	END IF
	...
	IF p_rec_search.text3 IS NOT NULL THEN
		...
		ELSE
			LET l_ret_tmp_str = trim(l_ret_tmp_str), "|", trim(p_rec_search.text3)
		END IF
	END IF
	...
	IF p_rec_search.text4 IS NOT NULL THEN
		...
		ELSE
			LET l_ret_tmp_str = trim(l_ret_tmp_str), "|", trim(p_rec_search.text4)
		END IF
	END IF
...
FUNCTION search(p_rec_param)
	...
	DEFINE l_rec_addr OF dt_rec_util_address_retrieve_output  #Final field based Address	

	#Reset previous address retrieve result / clear fields with NULLs
	INITIALIZE l_rec_addr TO NULL		
	CALL display_address(l_rec_addr.*)

	#fire off service request
	CALL util_address_search(p_rec_param.*) RETURNING l_arr_rec_addr
	#CALL search_address(p_rec_param.*) RETURNING l_arr_rec_addr	#hand coded way of doing it... but makes no sense
	...
	AND (l_arr_rec_addr[1].type == "Address") THEN #type address is if it actually finds the address / otherwise it's a list array of possible addresses
		CALL util_address_retrieve(l_arr_rec_addr[1].id,p_rec_param.license_key) RETURNING l_rec_addr.*
...
FUNCTION init_param()
	...
	LET l_rec_param.license_key = LICENSE #"YR96-DP94-XE91-HY91" # "TZ78-EF47-BN68-RT67" #The key used to authenticate with the service. "CB99-MF88-XZ97-NJ73" "ZZ99-JB69-EP69-WR53"
	#LET l_rec_param.license_key = "CB99-MF88-XZ97-NJ73" #The key used to authenticate with the service. "CB99-MF88-XZ97-NJ73" "ZZ99-JB69-EP69-WR53"
...
#####################################################################################
FUNCTION display_address(p_rec_addr)
	DEFINE p_rec_addr OF dt_rec_util_address_retrieve_output  #Final field based Address	
	...
	DISPLAY p_rec_addr.type TO type		 						#		The type of address if known.
