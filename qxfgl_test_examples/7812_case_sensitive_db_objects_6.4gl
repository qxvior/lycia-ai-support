...
MAIN
   ...
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "1. SQLCA.SQLCODE = ",SQLCA.SQLCODE EXIT PROGRAM END IF
	...
	DECLARE cur CURSOR FOR
		...
		WHERE syscolumns.tabid = systables.tabid AND
				systables.tabname = "lyc_7812_table" 
		ORDER BY syscolumns.colno

	FOREACH cur INTO v_colname
   	IF SQLCA.SQLCODE <> 0 THEN DISPLAY "2. SQLCA.SQLCODE = ",SQLCA.SQLCODE ELSE DISPLAY "2. ",v_colname END IF
	END FOREACH
   ...
   EXECUTE p_create
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "3. SQLCA.SQLCODE = ",SQLCA.SQLCODE EXIT PROGRAM END IF

	FOREACH cur INTO v_colname
   	IF SQLCA.SQLCODE <> 0 THEN DISPLAY "4. SQLCA.SQLCODE = ",SQLCA.SQLCODE ELSE DISPLAY "4. ",v_colname END IF
	END FOREACH
