...
MAIN
   ...
   (row_number SERIAL,  
    col_char CHAR(80),
    PRIMARY KEY (row_number) CONSTRAINT lyc_7812_table_primary_key_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz
   )
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "1. SQLCA.SQLCODE = ",SQLCA.SQLCODE EXIT PROGRAM END IF

	SELECT constrname INTO v_constrname FROM sysconstraints WHERE constrname = "lyc_7812_table_primary_key_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
