...
MAIN
   ...
   CREATE PRIVATE SYNONYM lyc_7812_private_synonym_1_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz FOR lyc_7812_table
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "2. SQLCA.SQLCODE = ",SQLCA.SQLCODE EXIT PROGRAM END IF

	SELECT tabname INTO v_tabname FROM systables WHERE tabname = "lyc_7812_private_synonym_1_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
   ...
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "4. SQLCA.SQLCODE = ",SQLCA.SQLCODE ELSE DISPLAY "4. ",v_tabname END IF
	...
	LET v_prep = "CREATE PRIVATE SYNONYM lyc_7812_private_synonym_2_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz FOR lyc_7812_table"
   PREPARE p_create FROM v_prep
   EXECUTE p_create
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "5. SQLCA.SQLCODE = ",SQLCA.SQLCODE EXIT PROGRAM END IF

	SELECT tabname INTO v_tabname FROM systables WHERE tabname = "lyc_7812_private_synonym_2_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
   ...
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "7. SQLCA.SQLCODE = ",SQLCA.SQLCODE ELSE DISPLAY "7. ",v_tabname END IF
	...
	SQL
   CREATE PRIVATE SYNONYM lyc_7812_private_synonym_3_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz FOR lyc_7812_table
	END SQL

	SELECT tabname INTO v_tabname FROM systables WHERE tabname = "lyc_7812_private_synonym_3_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
