...
# IMPORT
##################################################################################
IMPORT SWAGGER "../currencyExchangeSwagger.json" AS currency_swag
...
DEFINE res	#Vlad's special structured record 
	RECORD
		stat INT,
		currency currency_swag.responses_result
	END RECORD
...
MAIN
  ...
  DISPLAY "Currency Exchange Rate Converter (Swagger WEB-API Demo)" TO lbTitle
...
FUNCTION PopulateExchange()
  ...
  FOR i = 1 TO currencies.GetSize()
    IF currencies[i].use AND currencies[i].currency <> base THEN
      CALL currency_swag.date_get(dt USING "yyyy-mm-dd", base, currencies[i].currency) RETURNING res.stat, res.currency.*
      CALL currencyExchange.Append([currencies[i].currency, res.currency.a.rate])
      --DISPLAY res.currency.bs, res.currency.dt, res.currency.a.amount
    END IF
  END FOR
END FUNCTION
...
FUNCTION Calc(from, to, amount)  
  ...
  CALL currency_swag.latest_get(from, to) RETURNING res.stat, res.currency.*
  LET result = amount * res.currency.a.rate
