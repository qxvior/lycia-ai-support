...
DEFINE	request_mode,date_str STRING
	...
	LET resultat = send.traitementChar(sms_recipients)

	LET resultat = send.OctopusSend(
		urljava,userlogjava,apikeyjava,smstextjava,
		smsrecjava,sms_mode,smstypejava,
		smsenderjava, request_mode)

	LET resultat = send.OctopusSendDif(
		urljava,userlogjava,apikeyjava,smstextjava,
		smsrecjava,sms_mode,smstypejava,
		smsenderjava,sms_d,sms_m,sms_y,sms_h,sms_i,
		request_mode)
END MAIN
