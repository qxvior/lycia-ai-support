...
MAIN
   ...
   (row_number SERIAL PRIMARY KEY CONSTRAINT lyc_7812_table_primary_key,  
    col_char CHAR(80)
   )
   ...
   (row_number INTEGER,  
    col_char CHAR(80),
    FOREIGN KEY (row_number) REFERENCES lyc_7812_table CONSTRAINT lyc_7812_table_ref_foreign_key_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz
   )
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "2. SQLCA.SQLCODE = ",SQLCA.SQLCODE EXIT PROGRAM END IF

	SELECT constrname INTO v_constrname FROM sysconstraints WHERE constrname = "lyc_7812_table_ref_foreign_key_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
