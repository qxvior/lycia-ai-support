...
MAIN
   ...
   END FOR      
   SQL INSERT INTO lyc_0436(col_lvarchar) VALUES($ins_lvarchar) END SQL
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "1.  SQLCA.SQLCODE = ",SQLCA.SQLCODE END IF
   LET sel_lvarchar = NULL
   SQL SELECT col_lvarchar INTO $sel_lvarchar FROM lyc_0436 WHERE col_lvarchar = $ins_lvarchar END SQL
   ...
   SQL INSERT INTO lyc_0436(col_lvarchar_1) VALUES($ins_lvarchar_1) END SQL
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "4.  SQLCA.SQLCODE = ",SQLCA.SQLCODE END IF
   LET sel_lvarchar_1 = NULL
   SQL SELECT col_lvarchar_1 INTO $sel_lvarchar_1 FROM lyc_0436 WHERE col_lvarchar_1 = $ins_lvarchar_1 END SQL
   ...
   END FOR      
   DELETE FROM lyc_0436
   SQL INSERT INTO lyc_0436(col_lvarchar_8000) VALUES($ins_lvarchar_8000) END SQL
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "7.  SQLCA.SQLCODE = ",SQLCA.SQLCODE END IF
   LET sel_lvarchar_8000 = NULL
   SQL SELECT col_lvarchar_8000 INTO $sel_lvarchar_8000 FROM lyc_0436 WHERE col_lvarchar_8000 = $ins_lvarchar_8000 END SQL
   IF SQLCA.SQLCODE <> 0 THEN DISPLAY "8.  SQLCA.SQLCODE = ",SQLCA.SQLCODE END IF
   IF sel_lvarchar_8000 IS NULL   THEN DISPLAY "9.  NULL"   ELSE DISPLAY "9.  |",sel_lvarchar_8000,"|"   END IF
